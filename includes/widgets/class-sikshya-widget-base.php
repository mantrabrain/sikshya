<?php
if (!class_exists('Sikshya_Widget_Base')) {

	abstract class Sikshya_Widget_Base extends WP_Widget
	{
				...
				case "radio":
					...
					<p>
                        ...
                        <input class="widefat"
							   ...
							   name="<?php echo esc_attr($this->get_field_name($field_key)); ?>"
							   type="radio"
							   value="<?php echo esc_attr($choice_key); ?>" <?php echo $value == $choice_key ? 'checked="checked"' : '';
						echo $extra_attribute_text; ?>/>
                        <label
							for="<?php echo esc_attr($this->get_field_id($field_key)) . '_' . esc_attr($choice_key); ?>"><?php echo esc_html($choice); ?>
                        </label>
		...
		public
		function get_extra_attribute_text($extra_attributes = array())
		{
		...
		function update($new_instance, $old_instance)
		{
			...
			foreach ($widget_fields as $field_key => $field) {
				...
				$instance[$field['name']] = Sikshya_Widget_Validation::instance()->sanitize($new_field_value, $field);
