...
class Sikshya_Module_Section_Logs
{
    ...
    {
        return Sikshya_Log_Handler_File::get_log_files();
    }
    ...
    public static function remove_log()
    {
        if (empty($_REQUEST['_wpnonce']) || !wp_verify_nonce(wp_unslash($_REQUEST['_wpnonce']), 'remove_log')) { // WPCS: input var ok, sanitization ok.
            wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'sikshya'));
        }

        if (!empty($_REQUEST['handle'])) {  // WPCS: input var ok.
            $log_handler = new Sikshya_Log_Handler_File();
            $log_handler->remove(wp_unslash($_REQUEST['handle'])); // WPCS: input var ok, sanitization ok.
        }

        wp_safe_redirect(esc_url_raw(admin_url('admin.php?page=sikshya-status&tab=logs')));
        exit();
    }
    ...
    public static function status_logs_file_template()
    {
        ...
        include SIKSHYA_PATH . '/includes/modules/status/templates/html-admin-page-status-logs.php';
    }

    private static function flush_db_logs()
    {
        if (empty($_REQUEST['_wpnonce']) || !wp_verify_nonce($_REQUEST['_wpnonce'], 'sikshya-status-logs')) { // WPCS: input var ok, sanitization ok.
            wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'sikshya'));
        }
        Sikshya_Log_Handler_DB::flush();
    ...
    private static function log_table_bulk_actions()
    {
        ...
        if ((isset($_REQUEST['action']) && 'delete' === $_REQUEST['action']) || (isset($_REQUEST['action2']) && 'delete' === $_REQUEST['action2'])) { // WPCS: input var ok, sanitization ok.
            Sikshya_Log_Handler_DB::delete($log_ids);
            wp_safe_redirect(esc_url_raw(admin_url('admin.php?page=sikshya-status&tab=logs')));
            exit();
        }
    }
    ...
    {
        if (!empty($_REQUEST['sikshya-flush-logs'])) { // WPCS: input var ok, CSRF ok.
            self::flush_db_logs();
        }
    ...
    {
        include_once SIKSHYA_PATH . '/includes/modules/status/list-tables/class-sikshya-admin-log-list-table.php';

        $log_table_list = new Sikshya_Admin_Log_List_Table();
        $log_table_list->prepare_items();

        include SIKSHYA_PATH . '/includes/modules/status/templates/html-admin-page-status-logs-db.php';
    }

    public static function log_template()
    {
        $log_options = $log_options = get_option('sikshya_log_options', 'db');

        if ($log_options === 'db') {

            $log_handler = 'Sikshya_Log_Handler_DB';
        } else {
            $log_handler = 'Sikshya_Log_Handler_File';
        }

        if ('Sikshya_Log_Handler_DB' === $log_handler) {
