...
class Sikshya_Admin_List_Table_Orders extends Sikshya_Admin_List_Table
{
	...
	public function define_columns($columns)
	{
		...
		$show_columns['order_id'] = __('OrderID', 'sikshya');
		$show_columns['course'] = __('Course', 'sikshya');
		$show_columns['student'] = __('Student', 'sikshya');
		$show_columns['order_amount'] = __('Order Amount', 'sikshya');
		$show_columns['order_status'] = __('Status', 'sikshya');
		$show_columns['date'] = __('Order Date', 'sikshya');
	...
	protected function prepare_row_data($post_id)
	{
		...
		$this->order_meta = get_post_meta($post_id, 'sikshya_order_meta', true);
	...
	protected function render_order_amount_column()
	{

		$currency = isset($this->order_meta['currency']) ? $this->order_meta['currency'] : sikshya_get_active_currency();

		$currency_symbol = sikshya_get_currency_symbols($currency);

		$total_amount = isset($this->order_meta['total_order_amount']) ? $this->order_meta['total_order_amount'] : 0;

		if ($total_amount > 0) {

			echo sikshya_get_price_with_symbol($total_amount, $currency_symbol);
		} else {
			echo '<span>' . __('FREE', 'sikshya');
		}
	}
	...
	protected function render_order_status_column()
	{
		...
		$status_id = $this->object->post_status;

		echo '<span class="order-' . esc_attr($status_id) . '">' . sikshya_get_order_statuses($status_id) . '</span>';
	}
