...
final class CertificateRenderer
{
    ...
    public static function wrap(string $inner, string $title, int $template_id = 0, string $share_url = '', string $hash = '', bool $show_controls = true, string $meta_line = ''): string
    {
        ...
        if ($show_controls) {
            ...
            . 'if(a==="copy-hash"){if(!hash){toast("No verification ID found.");return;}copyText(hash).then(function(ok){toast(ok?"Copied verification ID.":"Could not copy verification ID.");});return;}'
            . 'if(a==="copy-link"){copyText(shareUrl).then(function(ok){toast(ok?"Copied link.":"Could not copy link.");});return;}'
